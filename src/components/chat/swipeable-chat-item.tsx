...
export default function SwipeableChatItem(props: SwipeableChatItemProps) {
  ...
  const [shouldShowTutorialHint, setShouldShowTutorialHint] = useState(false);
  ...
  // Show hint for users who haven't interacted yet
  useEffect(() => {
    if (!hasInteracted && hasActiveConversation && conversation) {
  ...
  }, [hasInteracted, hasActiveConversation, conversation]);

  // Handle mouse enter to show subtle hint
  ...
  return (
    ...
    >
      {/* Swipe Hint Overlay */}
      {showHint && hasActiveConversation && !isDragging && (
        <div className="absolute inset-0 bg-black/5 backdrop-blur-[1px] z-20 flex items-center justify-center">
          <div className="bg-white/90 backdrop-blur-sm rounded-lg px-4 py-2 shadow-lg border border-gray-200/50 animate-pulse">
